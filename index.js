...
app.get('/sample', function (req, res) {
  ...
  parsedHTML('p b span').map(function(i, element) {
    ...
    if (refNo.indexOf('Reference No:') != -1) {
      ...
      var title          = nextElement.text(); nextElement = nextElement.next(); // etc
      var size           = nextElement.text(); nextElement = nextElement.next();
      ...
      csvObj.forEach(function(csvItem) {
        ...
        docObj.forEach(function(docItem) {
          ...
          for (var key in csvTitleRefs) {
            ...
            if (key === 'length' || !csvTitleRefs.hasOwnProperty(key)) continue;
